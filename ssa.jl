...
export ssa_decompose, ssa_reconstruct, reconstruct_past
...
function reconstruct_past(x::Array{float_type, dim}, M, eig_vecs, modes; sum_modes=false) where {float_type<:AbstractFloat} where dim
   if (dim == 1)
      # Single-channel SSA
      N = length(x)
      D = 1
   ...
   r = Array{float_type, 3}(undef, length(modes), D, J)
   ...
   for (i_k, k) in enumerate(modes)
      ...
      for j = 1:J
         ...
         U_n = M
         for d=1:D
            r[i_k, d, j] = 1/M_n*sum([A[n - m + 1, k]*ek[m, d] for m=L_n:U_n])
         end
      end
   end
   ...
   if sum_modes
      r = copy(selectdim(sum(r, dims=1), 1))
   end
